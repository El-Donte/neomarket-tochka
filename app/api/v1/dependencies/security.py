...
def create_refresh_token(seller_id: UUID) -> str:
    return create_token(
        seller_id,
        "refresh",
...
def decode_token(token: str, expected_type: str) -> Optional[UUID]:
    try:
...
def set_auth_cookies(response: Response, seller_id: UUID):
    access_token = create_access_token(seller_id)
    refresh_token = create_refresh_token(seller_id)
